"""PaperMC: the Fill v3 build listings a Paper plugin target is built against.

Readiness for Paper is a *build*, not a version: ``/v3/projects/paper`` lists the game
versions Paper knows, and only ``/v3/projects/paper/versions/<v>/builds`` says whether any
of them has a build and on which channel (``STABLE``, ``BETA``, ``ALPHA``). The per-version
endpoint returns build ids without a channel or a time, which is not enough to choose one,
so it is never read.
...
This module owns observation only. Acquiring a Paper server jar as a build input is a
