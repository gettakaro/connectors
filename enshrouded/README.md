...
enshrouded_server.exe (Wine/Proton, container)
  └─ dbghelp.dll  ← plugin/ (C++17, MinHook). Proxy DLL loaded by the server.
       ...
       └─ HTTP API on 127.0.0.1:18890 (Bearer token), see plugin/docs/API.md
...
- `plugin/`: `src/` source, `third_party/minhook` (vendored, see VENDORED.txt), `tools/gen_gamedata.py` (item/entity/location tables from the server kfc), `tests/` (host-side correlator test).
...
./plugin/build.sh                 # -> plugin/build/dbghelp.dll
./plugin/tests/run.sh             # host-side plugin tests (needs docker)

# sidecar
cd sidecar && npm ci && npm run typecheck && npm test && npm run build
```

`DEBUG_CORRUPT_SIG=<signature name> ./plugin/build.sh` builds a debug DLL into `plugin/build-debug/` with one
...
./plugin/build.sh
mkdir -p data/enshrouded-plugin && cp plugin/build/dbghelp.dll data/enshrouded-plugin/
