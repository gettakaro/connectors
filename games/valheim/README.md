...
- `mod/src/Takaro.Valheim.Core` contains the game-independent protocol, configuration, models, and request dispatcher.
- `mod/src/Takaro.Valheim.Companion.Protocol` contains the bounded shared wire contract.
- `mod/src/Takaro.Valheim.Plugin` contains the dedicated-server BepInEx adapter.
- `mod/src/Takaro.Valheim.Companion` contains the owned graphical-client BepInEx companion. It references only the protocol project and has no Takaro cloud transport.
