    ...
    public class LogService : IService
    {
        ...
        public void Debug(string message, string dir = "")
        {
            if (!LogDebug)
                return;
            Log.Out(
                $"[{API.ModPrefix}] *DEBUG* {(string.IsNullOrEmpty(dir) ? "" : dir + ": ")}{message}"
            );
        ...
        {
            Log.Out(
                $"[{API.ModPrefix}] *INFO* {(string.IsNullOrEmpty(dir) ? "" : dir + ": ")}{message}"
            );
        ...
        {
            Log.Out(
                $"[{API.ModPrefix}] *WARN* {(string.IsNullOrEmpty(dir) ? "" : dir + ": ")}{message}"
            );
        ...
        public void Error(string message, string dir = "")
        {
            ...
            Log.Out(
                $"[{API.ModPrefix}] *ERROR* {(string.IsNullOrEmpty(dir) ? "" : dir + ": ")}{message}"
            );
