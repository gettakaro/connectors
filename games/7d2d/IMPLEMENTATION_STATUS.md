...
Read requests (mirror-backed, `mod/src/WebSocket/ReadHandlers.cs`):
...
Action requests (main-thread dispatched, `mod/src/WebSocket/ActionHandlers.cs`):
...
All published via `mod/src/WebSocket/GameEventPublisher.cs` (non-blocking outbound queue):
