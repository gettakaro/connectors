...
PROJECT="mod/TakaroTerrariaEvents/TakaroTerrariaEvents.csproj"
DOTNET_IMAGE="${TERRARIA_DOTNET_IMAGE:-mcr.microsoft.com/dotnet/sdk:9.0}"

if [ ! -f "$REFS_DIR/TShockAPI.dll" ]; then
  echo "Missing TShock reference DLLs in games/terraria/$REFS_DIR" >&2
  echo "Run games/terraria/scripts/setup-environment.sh first." >&2
  exit 1
fi
...
echo "Built games/terraria/$OUT_DIR/TakaroTerrariaEvents.dll"
