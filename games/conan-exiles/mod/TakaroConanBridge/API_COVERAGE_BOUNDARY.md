...
The connector-owned Takaro surface is the Generic Connector game-server action/event protocol implemented in `bridge/src/takaro/protocol.ts`.
...
Every item above must have one explicit status in `bridge/src/takaro/coverage.ts` and `capabilities.json`: `live-supported`, `schema-fallback`, or `unsupported`.
