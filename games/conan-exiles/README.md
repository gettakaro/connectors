...
The code-level coverage registry lives in `bridge/src/takaro/coverage.ts`, and `bridge/src/__tests__/coverage.test.ts` fails if any Takaro action or event type is missing from that registry.
