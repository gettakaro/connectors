...
The companion implementation is automated-tested but its client-reported action/event paths remain `unsupported` until the exact packaged artifacts complete live server, client, and Takaro proof. See [COMPANION.md](COMPANION.md) for its trust boundary and operational guide.
...
| `getPlayerInventory` | `unsupported` | A negotiated companion can provide bounded canonical client-reported snapshots, including a confirmed empty inventory. The implementation still needs exact live proof; without a companion the server never fabricates `[]`. |
...
| `chat-message` | `unsupported` | Implemented as a negotiated client-reported path for ordinary local chat and configured commands, but not live-supported before exact end-to-end proof. |
| `player-death` | `unsupported` | Implemented as a bounded local-player client report with duplicate suppression; routed vanilla `OnDeath` diagnostics remain untrusted and non-emitting. |
| `entity-killed` | `unsupported` | Implemented as a client report only for non-player deaths whose cached last hit resolves the local player as attacker; exact live proof is still required. |
